<template>
  <v-card>
    <v-card-title class="justify-center">Modificar Cronograma</v-card-title>
        ...
        <v-row align="center" justify="space-around">
          <v-col cols="6" sm="5" md="4">
            ...
            >
              <template v-slot:activator="{ on, attrs }">
                <v-text-field
                  v-model="Cronograma.gestion.fecha_de_inicio_programada"
          ...
          <v-col cols="6" sm="5" md="4">
            ...
            >
              <template v-slot:activator="{ on, attrs }">
                <v-text-field
                  v-model="Cronograma.gestion.fecha_de_fin_programada"
                  label="Fecha de Fin"
                  prepend-icon="mdi-calendar"
                  readonly
                  v-bind="attrs"
  ...
  methods: {
    ...
    async actualizaFecha() {
      ...
      if (variable.modulo[0] == null) {
        ...
        variable.gestion.fecha_de_inicio_programada=this.Cronograma.gestion.fecha_de_inicio_programada;
        let listaModulos = [];
        let listaSubModulos = [];
        let listaActividades = [];
        listaModulos = variable.modulo;
        for (let i = 0; i < listaModulos.length; i++) {
          listaSubModulos = variable.modulo[i].submodulos;
          if (i == 0) {
            variable.modulo[i].fecha_inicio =variable.gestion.fecha_de_inicio_programada;
            for (let j = 0; j < listaSubModulos.length; j++) {
              listaActividades = variable.modulo[i].submodulos[j].actividades;
              if (j == 0) {
                variable.modulo[i].submodulos[j].fecha_inicio =variable.modulo[i].fecha_inicio;
                for (let k = 0; k < listaActividades.length; k++) {
                  if (k == 0) {
                    variable.modulo[i].submodulos[j].actividades[k].fecha_inicio =variable.modulo[i].submodulos[j].fecha_inicio;
                    var a =variable.modulo[i].submodulos[j].actividades[k].horas_estimadas;
                    var b = a / 8;
                    var c = new Date(variable.modulo[i].submodulos[j].actividades[k].fecha_inicio);
                    var f = 0;
                    f = Math.round(b);
                    c.setDate(c.getDate() + f);
                    variable.modulo[i].submodulos[j].actividades[k].fecha_fin =c;
                  } else {
                    variable.modulo[i].submodulos[j].actividades[k].fecha_inicio =variable.modulo[i].submodulos[j].actividades[k - 1].fecha_fin;
                    var a =variable.modulo[i].submodulos[j].actividades[k].horas_estimadas;
                    var b = a / 8;
                    var c = new Date(variable.modulo[i].submodulos[j].actividades[k].fecha_inicio);
                    var f = 0;
                  ...
                  variable.modulo[i].submodulos[j].fecha_fin =variable.modulo[i].submodulos[j].actividades[k].fecha_fin;
                }
              } else {
                 variable.modulo[i].submodulos[j].fecha_inicio =variable.modulo[i].submodulos[j-1].fecha_fin;
                for (let k = 0; k < listaActividades.length; k++) {
                  if (k == 0) {
                    variable.modulo[i].submodulos[j].actividades[k].fecha_inicio =variable.modulo[i].submodulos[j].fecha_inicio;
                    var a =variable.modulo[i].submodulos[j].actividades[k].horas_estimadas;
                    ...
                    variable.modulo[i].submodulos[j].actividades[k].fecha_inicio =
                      variable.modulo[i].submodulos[j].actividades[k - 1].fecha_fin;
                    var a =variable.modulo[i].submodulos[j].actividades[k].horas_estimadas;
                    var b = a / 8;
                    var c = new Date(variable.modulo[i].submodulos[j].actividades[k].fecha_inicio);
                    var f = 0;
                ...
                variable.modulo[i].fecha_fin=variable.modulo[i].submodulos[j].fecha_fin
            }
          } else {
            variable.modulo[i].fecha_inicio =variable.modulo[i-1].fecha_fin;
            for (let j = 0; j < listaSubModulos.length; j++) {
              listaActividades = variable.modulo[i].submodulos[j].actividades;
              if (j == 0) {
                variable.modulo[i].submodulos[j].fecha_inicio =
                  variable.modulo[i].fecha_inicio;
                for (let k = 0; k < listaActividades.length; k++) {
                  if (k == 0) {
                    variable.modulo[i].submodulos[j].actividades[
                      k
                    ].fecha_inicio =
                      variable.modulo[i].submodulos[j].fecha_inicio;
                    var a =
                      variable.modulo[i].submodulos[j].actividades[k]
                        .horas_estimadas;
                    var b = a / 8;
                    var c = new Date(
                      variable.modulo[i].submodulos[j].actividades[
                    ...
                    variable.modulo[i].submodulos[j].actividades[k].fecha_fin =
                      c;
                  } else {
                    variable.modulo[i].submodulos[j].actividades[
                      k
                  ...
                  variable.modulo[i].submodulos[j].fecha_fin =
                    variable.modulo[i].submodulos[j].actividades[k].fecha_fin;
                }
              } else {
                variable.modulo[i].submodulos[j].fecha_inicio=variable.modulo[i].submodulos[j-1].fecha_fin
          ...
          variable.gestion.fecha_de_fin_programada=variable.modulo[i].fecha_fin
         }
        variable.gestion.fecha_de_fin_programada=variable.modulo[i].fecha_fin
        
        }

        console.log("  variable.gestion.fecha_de_fin_programada")
     ...
     async modificarfechaCronograma() {
      ...
      await axios
        ...
        .catch((err) => console.log(err));
      if (this.Cronograma.gestion.fecha_de_inicio_programada == null) {
        await this.mensaje(
          "error",
          "Error",
    ...
    async estadoCronograma() {
      ...
      await axios
        ...
        .then((res) => {
          variable = res.data;
          this.estadoCrono = variable.estado;
        })
        .catch((err) => console.log(err));
    },
    condicionEstado() {
    ...
    async obtieneInfoCronogramaCreado() {
      ...
      await axios
        ...
        .then((res) => {
          ...
          variable = res.data;
        //  var a = new Date(this.variable.gestion.fecha_de_inicio_programada);
       
         variable.gestion.fecha_de_fin_programada = res.data.gestion.fecha_de_fin_programada.split("T")[0]
          this.Cronograma.gestion.fecha_de_fin_programada=variable.gestion.fecha_de_fin_programada
